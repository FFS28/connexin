...
import { CellType } from "../../../utils/customTypes/apiData";
...
function SearchInput({search}: {search: (param: string) => void}) {
    return (
...
function EnhancedTableHead(props: {order: 'asc' | 'desc', orderBy: string, onRequestSort: (param: string) => void, headCells: CellType[], alertBody: string, search: (param: string) => void}) {
    ...
    return (
        <>
            <TableHead>
                ...
                <TableRow >
                    {headCells.map((headCell: CellType) => (
...
const TPagination = ({ count, page, onPageChange }: { count: number, page: number, onPageChange: (param: number) => void}) => {
    const totalPages = Math.ceil(count / 10);
    return (
        <Box>
            <Grid container spacing={2}>
                <Grid item xs={3}>
                    <Typography variant={"subtitle1"} component={"h6"} sx={{ my: 2 }}>Page {page + 1} of {totalPages}</Typography>
                </Grid>
                <Grid item xs={6}>
                    <Stack sx={{justifyContent: "center", display: "flex", alignItems: "center", pt: 2}}>
                        <Pagination count={totalPages} page={page + 1} onChange={(event, pages)=>onPageChange(pages)} sx={{alignItems: "center", justifyContent: "center"}} />
                    </Stack>
                </Grid>
                <Grid item xs={3}>
                    <Typography variant={"subtitle1"} component={"h6"} sx={{ my: 2 }}>Total Items : {count}</Typography>
                </Grid>
            </Grid>
        </Box>
    )
...
export default function ContentTable({rows, headCells, childrenTag, search, handle} : {rows: any, headCells: CellType[], childrenTag: string, search: (param: string) => void, handle: (param: number) => void}) {
                    ...
                    <Table sx={{ minWidth: 750 }} aria-labelledby={"tableTitle"} size={dense ? 'small' : 'medium'} >
                        ...
                        <TableBody>
                            {stableSort(rows, getComparator(order, orderBy)).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row: any, index: number) => {
                                ...
                                return (
                                    <TableRow hover onClick={() => handleClick(index)} role={"checkbox"} aria-checked={isItemSelected} tabIndex={-1} key={index} selected={isItemSelected} >
